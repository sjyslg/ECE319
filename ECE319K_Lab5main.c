...
// I.e., if key has not changed DO NOT CALL start or stop
  key = Key_In();
    if((last == 0) && key != 0){
      uint32_t p = 0;
      if(key == 1) p = Key0; // set period p depending on input key
      else if (key == 2) p = Key1;
      else if (key == 4) p = Key2;
      else if (key == 8) p = Key3;
      Sound_Start(p);
      Debug_Dump(p);
    }
      else if ((last != 0) && (key == 0)) {
      Sound_Stop();
    }
    last = key;
    Clock_Delay(800000); // 10ms, to debounce switch
